...
if [ "${analyze_bool}" == true ]; then
    ...
    if [ ! -d "${tools_dir}/ad-recon" ]; then
        ...
        rm -rf "${tools_dir}/ad-recon/output"
        # Run ad-recon (writes to ./output/ by default)
        ${python3} ad_recon.py --pathing --transitive -U "bolt://127.0.0.1:${bolt_port}" -u neo4j -p "${neo4j_password}" -d neo4j
        # Copy results to destination
        cp -r "${tools_dir}/ad-recon/output/"* "${proj_out_dir}/ad-recon_${domain}/"
        echo -e ""
